...
|------|-------|-----------------|---------------|
| 2026-08-29 | Ports | Windows and macOS builds, verified by CI on real runners rather than asserted. The server and the SDL client build on both; the X11 client, replay tool and map editor are Xlib programs and are not built there. CMake now chooses per platform, uses the portable SDL game loop off X11, and finds SDL2 through either pkg-config or its own CMake config. Eight defects, none of them visible on Linux: Winsock was never included so socklen_t did not exist; `_WINDOWS` was an MSVC-only macro and is now derived from `_WIN32`; `ioctlsocket` needs a `u_long*` where POSIX `ioctl` takes an `int*`; `<direct.h>` was included only under `_MSC_VER` while `mkdir` was redefined for every Windows compiler; `Progress()` was declared only in the old Win32 GUI client's private header; `GL_CLAMP_TO_EDGE` is absent because Windows still ships the OpenGL 1.1 headers; nothing linked ws2_32; and macOS needed `<OpenGL/gl.h>` chosen independently of where SDL lives. Above all, **the Windows server could never have run** -- its timer installed a callback that exists nowhere in the tree, its tick consumer never called the frame handler, and its scheduler had no loop, so it started, loaded the map, announced its frame rate and exited. Only the runtime check in CI caught that; a build-only job would have called it a success. Also fixed a CI trigger that covered only `master` and `phase*`, so this branch pushed untested and looked healthy. | `port-windows-macos` |
